...
Calculate_paired_correlation_from_matrix <- function(logFC_matrix,pvalue_matrix,number_top=F,method="spearman"){
  ...
  corr_matrix_list <- list(corr=correlation_matrix,P.Value=correlation_matrix,FDR=correlation_matrix,Zscore=correlation_matrix)
  ...
  for (i in 1:(nrow(correlation_matrix)-1)){
    ...
    for (j in (i+1):(ncol(correlation_matrix))){
      ...
      temp_data_logFC <- logFC_matrix[,c(name1,name2)]
      temp_data_logFC <- temp_data_logFC[complete.cases(temp_data_logFC),]
  ...
  FDR_table <- as.data.frame(matrix(p.adjust(as.vector(FDR_table),method="BH"),nrow=nrow(corr_matrix_list$P.Value)))
