...
Correlation_loop_top_genes <- function(logFC_table,pvalue_table,
                                       vector_number_genes,
                                       method="pearson",
                                       FDR_thresh=0.05,
                                       corr_thresh=0.1,
  ...
  #Select samples for correlation matrix
  if (samplenames==F){
  ...
  source("FUN_Calculate_paired_correlation_from_matrix.r")
  ...
  for (number_genes in vector_number_genes){
    ...
    significant_pairs$FDR <- c(significant_pairs$FDR,
                               sum(temp_corr_list$FDR<FDR_thresh)-nrow(temp_corr_list$FDR))
